...
is system([0,3], $cqctl, 'status'), 3, 'cqctl status stopped exit';
like qx($cqadm status), qr/not running/, 'cqadm status stopped';
is system([0,3], $cqadm, 'status'), 3, 'cqadm status stopped exit';
...
is system([0,3], $cqctl, 'status'), 0, 'cqctl status running exit';
like qx($cqadm status), qr/ready/, 'cqadm status ready';
is system([0,3], $cqadm, 'status'), 0, 'cqadm status ready exit';
